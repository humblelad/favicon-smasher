...
app.config['MAX_CONTENT_LENGTH'] = 1 * 1024 * 1024  # 1 MB in bytes

# A global variable to store the file path
...
def index():
    ...
    if request.method == 'POST':
        ...
        timestamp = str(int(time.time()))  # Using the current Unix timestamp
        filename = secure_filename(timestamp + '.txt')
        ...
        # Schedule a task to delete the file after 10 minutes
        delete_task = threading.Timer(600, delete_uploaded_file)
        delete_task.start()


        m = compute()

        return render_template('d1.html', matches=m)
...
def compute():
    ...
    matches = []  # Collect matching results

    if uploaded_file_path:
        # Read the domains from the uploaded file
        with open(uploaded_file_path, 'r') as f:
            lines = f.read().splitlines()

        for line in lines:
            # Strip leading and trailing whitespace and check if the line is not empty
            line = line.strip()
            if not line:
                continue  # Skip empty lines
            # Check if the line starts with "http://" or "https://"
            if line.startswith("http://") or line.startswith("https://"):
                domain = line.lstrip("http://").lstrip("https://").lstrip(":").strip()

                try:
                    response = requests.get(f'{line}/favicon.ico')
                    response.raise_for_status()  # Raise an error for non-2xx responses
                    favicon = codecs.encode(response.content, "base64")
                    hash = mmh3.hash(favicon, signed=True)
                    print(f"Computed hash for {domain}: {hash}")

                    if hash in fingerprint:
                        print(f"Hash {hash} matched with {fingerprint[hash]} for domain {domain}")
                        matches.append((domain, hash))  # Add matching result to the list
                    else:
                        print(f"No match for domain {domain}")
                except (requests.exceptions.RequestException, requests.exceptions.HTTPError) as e:
                    print(f"Failed to load domain {domain}: {str(e)}")
            else:
                print(f"Invalid domain: {line} (must start with 'http://' or 'https://')")
